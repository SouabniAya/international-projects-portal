...
<div class="pm-page">
    ...
    <div class="pm-toolbar">
        <span class="pm-toolbar__count">Projects ({{ $projects->total() }})</span>
                ...
                <label class="pm-toolbar__sort">
                    ...
                    <div class="pm-toolbar__select">
                        <select name="sort" onchange="this.form.submit()">
                            ...
                            <option value="deadline" @selected($sort === 'deadline')>End Date</option>
    ...
    <div class="pm-layout">
        ...
        <div class="pm-table-wrap" data-pm-view="table">
            <table class="pm-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="pm-table__actions">
                                <a href="{{ route('admin.project-details', $project['projectID']) }}" aria-label="View">
                    ...
                    @empty
                    <tr>
                        <td colspan="9" style="text-align:center; padding:32px;">No projects yet.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="pm-pagination">
                <span>Showing {{ $projects->firstItem() ?? 0 }} to {{ $projects->lastItem() ?? 0 }} of {{ $projects->total() }} projects</span>
                <div class="pm-pagination__buttons">
                    <button type="button" aria-label="Previous page" @if(!$projects->onFirstPage()) onclick="window.location='{{ $projects->previousPageUrl() }}'" @else disabled @endif>‹</button>
                    ...
                    @endif
                    <button type="button" aria-label="Next page" @if($projects->hasMorePages()) onclick="window.location='{{ $projects->nextPageUrl() }}'" @else disabled @endif>›</button>
                </div>
            </div>
        </div>

        {{-- Board (card) view — same $projects collection, alternate layout --}}
            ...
            <div class="pm-board">
                ...
                <div class="pm-board__card">
                    <div class="pm-board__card-head">
                        <img src="{{ asset($project['logo']) }}" alt="">
                        <span class="pm-table__status pm-table__status--{{ \Illuminate\Support\Str::slug($project['status']) }}">{{ $project['status'] }}</span>
                    </div>
                    ...
                    <dl class="pm-board__meta">
                        <div><dt>Coordinator</dt><dd>{{ $project['coordinator'] }}</dd></div>
                        <div><dt>Partners</dt><dd>{{ $project['partners'] }}</dd></div>
                        <div><dt>Period</dt><dd>{{ $project['start'] }} – {{ $project['end'] }}</dd></div>
                        <div><dt>Budget</dt><dd>{{ $project['budget'] }}</dd></div>
                    </dl>
                    <a href="{{ route('admin.project-details', $project['projectID']) }}" class="pm-board__link">View details →</a>
                </div>
                @empty
                <p style="padding:32px; text-align:center;">No projects yet.</p>
                @endforelse
            </div>
        ...
        <aside class="pm-sidebar">
            ...
            <div class="pm-sidebar__panel">
                ...
                {{--
                    No Deadline/Milestone table exists anywhere in the schema
                    (Project only has startDate/endDate) — there is nothing
                    real to query here yet, so this is an honest empty state
                    rather than fabricated deadline data. If per-project
                    deliverable deadlines are wanted, this needs a new table
                    (e.g. ProjectDeadline: projectID, title, dueDate).
                --}}
            ...
            <div class="pm-sidebar__panel">
                ...
                <ul class="pm-sidebar__actions">
                    ...
                    {{--
                        Project Templates / Import Project / Manage Templates /
                        Project Categories: none of these correspond to any
                        real feature in the schema or specs — left out rather
                        than wired to dead links. Tell me if any of these
            ...
            <div class="pm-sidebar__panel">
                ...
                <ul class="pm-sidebar__actions">
                    ...
                    {{--
                        Project Performance Report / Financial Overview: no
                        report-generation backend exists — same as the
                        "Reports" admin page question flagged earlier.
                        Left out rather than wired to dead links.
                    --}}
                </ul>
            </div>
