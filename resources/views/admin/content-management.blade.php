...
<div class="section__header section__header--row" style="margin-bottom:8px;">
    <div>
        ...
        <p style="margin:4px 0 0;">{{ __('Create, edit, and publish content across the portal.') }}</p>
    </div>
    <button type="button" class="btn btn--primary btn--sm" data-modal-open="newContentModal">+ {{ __('New Content') }}</button>
</div>
<p style="margin:0 0 20px; font-family:var(--font-body); font-size:12.5px;">
    {{ __('Prefer a full page?') }} <a href="{{ url('/admin/content/create') }}" style="color:var(--color-cerulean);">{{ __('Use the dedicated Publish Content page →') }}</a>
</p>
...
<div class="flex-row" style="margin-bottom:20px; border-bottom:1px solid var(--color-neutral-300); padding-bottom:12px;">
    ...
    <a href="#" class="btn {{ $i === 0 ? 'btn--primary' : 'btn--outline' }} btn--sm">{{ __($tab) }}</a>
    @endforeach
</div>
...
<div class="filter-bar" style="margin-bottom:20px;" data-filter-scope="#contentTableWrap">
    <div class="filter-bar__search">
        ...
        <input type="search" placeholder="{{ __('Search content...') }}" data-filter-search>
    </div>
    ...
    <button type="button" class="btn btn--outline btn--sm" data-export-csv="#contentTableWrap" data-export-filename="content-management">⭳ {{ __('Export CSV') }}</button>
</div>

<div class="data-table-wrap" id="contentTableWrap">
    <table class="data-table">
        <thead>
            <tr><th>{{ __('Title') }}</th><th>{{ __('Type') }}</th><th>{{ __('Status') }}</th><th>{{ __('Author') }}</th><th>{{ __('Last Modified') }}</th><th></th></tr>
        </thead>
        <tbody>
            @foreach ([
            ...
            <tr data-status="{{ $item['status'] }}" data-type="{{ $item['type'] }}">
                <td data-row-title>{{ __($item['title']) }}</td>
                <td><span class="pill pill--outline">{{ __($item['type']) }}</span></td>
                <td><span class="badge badge--{{ $item['status'] }}">{{ __($item['label']) }}</span></td>
                <td>{{ $item['author'] }}</td>
                <td>{{ $item['date'] }}</td>
                <td class="data-table__actions">
                    <button type="button" data-action="preview" aria-label="{{ __('Preview') }}">
                    ...
                    <button type="button" data-action="edit" aria-label="{{ __('Edit') }}">
                    ...
                    <button type="button" data-action="archive" aria-label="{{ __('Archive') }}">
                    ...
                    <button type="button" data-action="delete" aria-label="{{ __('Delete') }}">
...
<p data-empty-state style="display:none; text-align:center; padding:32px; color:var(--color-neutral-500); font-family:var(--font-body);">{{ __('No content matches your filters.') }}</p>

<nav class="pagination" aria-label="{{ __('Content pages') }}">
...
<dialog id="newContentModal" class="modal" style="max-width:560px;">
    <form method="POST" action="{{ route('admin.content.store') }}" data-demo-submit="{{ __('New content saved as draft (demo — connect this route to persist).') }}">
        @csrf
        <div class="modal__header">
            <h3>{{ __('New Content') }}</h3>
            <button type="button" class="modal__close" data-modal-close aria-label="{{ __('Close') }}">&times;</button>
        </div>
        <div class="modal__body">
            <div class="form-group">
                <label class="form-label" for="newContentType">{{ __('Content type') }}</label>
            ...
            <div class="form-group">
                <label class="form-label" for="newContentTitle">{{ __('Title') }}</label>
            ...
            <div data-fields-for="News">
                <div class="form-group">
                    <label class="form-label" for="newsBody">{{ __('Article body') }}</label>
                    <textarea class="form-control" id="newsBody" name="body" rows="4"></textarea>
                </div>
                <div class="form-group">
                    <label class="form-label" for="newsImage">{{ __('Cover image') }}</label>
            ...
            <div data-fields-for="Event" style="display:none;">
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label" for="eventStart">{{ __('Start date/time') }}</label>
                        <input class="form-control" type="datetime-local" id="eventStart" name="start_date">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="eventEnd">{{ __('End date/time') }}</label>
            ...
            <div data-fields-for="Project" style="display:none;">
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label" for="projectAcronym">{{ __('Acronym') }}</label>
                        <input class="form-control" type="text" id="projectAcronym" name="acronym">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="projectProgramme">{{ __('Funding programme') }}</label>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label" for="projectStart">{{ __('Start date') }}</label>
                        <input class="form-control" type="date" id="projectStart" name="start_date">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="projectEnd">{{ __('End date') }}</label>
                        <input class="form-control" type="date" id="projectEnd" name="end_date">
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label" for="projectAbstract">{{ __('Abstract') }}</label>
            ...
            <div data-fields-for="Call for Proposal" style="display:none;">
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label" for="callOpening">{{ __('Opening date') }}</label>
                        <input class="form-control" type="date" id="callOpening" name="opening_date">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="callDeadline">{{ __('Deadline') }}</label>
                        <input class="form-control" type="date" id="callDeadline" name="deadline">
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label" for="callBudget">{{ __('Budget') }}</label>
                    <input class="form-control" type="text" id="callBudget" name="budget" placeholder="{{ __('e.g. €50,000') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="callDescription">{{ __('Description') }}</label>
            ...
            <div data-fields-for="Mobility Opportunity" style="display:none;">
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label" for="mobilityHost">{{ __('Hosting establishment') }}</label>
                        <input class="form-control" type="text" id="mobilityHost" name="host">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="mobilityPlaces">{{ __('Places available') }}</label>
                        <input class="form-control" type="number" id="mobilityPlaces" name="places" min="1">
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label" for="mobilityDeadline">{{ __('Application deadline') }}</label>
                    <input class="form-control" type="date" id="mobilityDeadline" name="application_deadline">
                </div>
            </div>

            <div class="form-group">
                <label class="form-label" for="newContentStatus">{{ __('Publication status') }}</label>
                <select class="form-control" id="newContentStatus" name="publication_status">
                    <option value="draft">{{ __('Save as draft') }}</option>
                    <option value="scheduled">{{ __('Schedule for later') }}</option>
                    <option value="published">{{ __('Publish now') }}</option>
                </select>
            </div>
        </div>
